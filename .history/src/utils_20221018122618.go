...
func Sec2Diff(sec uint64) string {
	...
	if uptime >= day {
		...
		return fmt.Sprintf("%v days %v h %v m %v s", days, hours, minutes, seconds)
	} else if uptime >= hour {
		hours := uptime / hour
		a := uptime % hour
		minutes := a / min
		...
		fmt.Printf("minutes: %v\n", minutes)
		fmt.Printf("seconds: %v\n", seconds)
		return fmt.Sprintf("%v h %v m %v s", hours, minutes, seconds)
	} else if uptime >= min {
		minutes := uptime / min
		a := uptime % min
		seconds := a % min
		fmt.Printf("minutes: %v\n", minutes)
		fmt.Printf("seconds: %v\n", seconds)
		return fmt.Sprintf("%v m %v s", minutes, seconds)
	} else {
		fmt.Printf("seconds: %v\n", uptime)
		return fmt.Sprintf("%v s", uptime)
	}
}
